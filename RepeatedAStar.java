...
public class RepeatedAStar {
    ...
    static int experimentSize = 5;

    public static void main(String[] args){
        /* 
         * Singular testing for Repeated A* Forward, Repeated A* Backward, and Adaptive A*
         * Prints visualization of maze and optimal trajectory of agent to target\
         * 
         * Repeated A* Forward can either break ties of smallest f-value in favor of cells with:
         *   (0) smaller g-values
         *   (1) larger g-values
         */
        ...
        /*
         * Process builds multiple grid world environments (variable experimentSize)
         * Displays performance differences between each search algorithms
         */
        experiment();
    }
    ...
    public static void experiment() {
        int[][] stats = new int[experimentSize][4];
        int i = 0;
        while (i < experimentSize) {
        ...
        // Print costs of each experiement
        System.out.println("Experiment Costs");
        System.out.println("Forward A*(0)\tForwardA*(1)\tBackward A*\tAdaptive A*");
        for (int r = 0; r < experimentSize; r++) {
            for (int c = 0; c < 4; c++) {
        ...
        for (i = 0; i < experimentSize; i++){
            forwardSmallerAvg += stats[i][0];
            forwardHigherAvg += stats[i][1];
            backwardAvg += stats[i][2];
            adaptiveAvg += stats[i][3];
        }
        forwardSmallerAvg /= experimentSize;
        forwardHigherAvg /= experimentSize;
        backwardAvg /= experimentSize;
        adaptiveAvg /= experimentSize;
